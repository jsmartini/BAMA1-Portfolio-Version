...
def stepper_rpc(distance:float, brake: bool, spam: int):
    conn = connect(10, API["STEPPER"]["csp_service_id"])
    csp_send(
        conn = conn,
...
def arm_payload_rpc(password, spam: int):
    conn = connect(10, API["ARM"]["csp_service_id"])
    csp_send(
        conn = conn,
...
def deploy_payload_rpc(flag:bool, spam:int):
    conn = connect(10, API["DEPLOY"]["csp_service_id"])
    csp_send(
        conn = conn,
        data = [flag, spam],
...
def binary_command_rpc(cmd:int, spam:int):
    conn = connect(10, API["BIN"]["csp_service_id"])
    csp_send(
        conn = conn,
        data = [cmd, spam],
